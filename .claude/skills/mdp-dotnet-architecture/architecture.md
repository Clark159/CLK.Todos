...
本份.NET 設計規範，涵蓋 .NET 專案的目錄與分層結構（Workspace／Architecture）、程式碼撰寫慣例（Namespace／Class／Constructor／Method）、以及類別的設計規範（Context／Repository／Entity）。.NET專案應遵循本設計規範，進行架構設計規畫及程式碼撰寫開發；
